...
public class CommandSimplicity implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        Integer length = args.<Integer>getOne("length").orElse(5);
            ...
            case "title-sound":
                ...
                try {
                    if (player != null) {
                        ...
                        player.getPlayer().get().sendTitle(Title.builder().actionBar(TextSerializers.FORMATTING_CODE.deserialize(message)).stay(length).build());
                        return CommandResult.success();
                    } else {
                        for (Player player1 : Sponge.getServer().getOnlinePlayers()) {
                            player1.playSound(SoundType.of(sound), player1.getPosition(), 1.0f);
                            player1.sendTitle(Title.builder().actionBar(TextSerializers.FORMATTING_CODE.deserialize(message)).stay(length).build());
            ...
            case "title-chat-sound":
                ...
                try {
                    if (player != null) {
                        player.getPlayer().get().playSound(SoundType.of(sound), player.getPosition(), 1.0f);
                        player.getPlayer().get().sendMessage(TextSerializers.FORMATTING_CODE.deserialize(message));
                        player.sendTitle(Title.builder().actionBar(TextSerializers.FORMATTING_CODE.deserialize(message)).stay(length).build());
                        return CommandResult.success();
                    } else {
                        for (Player player1 : Sponge.getServer().getOnlinePlayers()) {
                            player1.playSound(SoundType.of(sound), player1.getPosition(), 1.0f);
